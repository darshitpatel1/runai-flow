...
// Initialize Firebase Admin only once with minimal configuration
if (!admin.apps.length) {
  try {
    // For testing, we can initialize with just the project ID
    admin.initializeApp({
      projectId: process.env.VITE_FIREBASE_PROJECT_ID || 'demo-project'
    });
    console.log('Firebase Admin SDK initialized with project ID:', process.env.VITE_FIREBASE_PROJECT_ID || 'demo-project');
  } catch (error) {
  ...
  async getExecutions(userId, queryParams = {}) {
    try {
      ...
      if (queryParams.limit) {
        queryRef = queryRef.limit(parseInt(queryParams.limit));
      }
