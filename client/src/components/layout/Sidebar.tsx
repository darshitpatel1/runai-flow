  ...
  return (
    <div className={`flex flex-col h-full transition-all duration-300 ${collapsed ? 'w-16' : 'w-64'}`}>
      ...
      <nav className="flex-1 py-4 space-y-1 overflow-y-auto">
        ...
        <Link href="/docs" className={`flex items-center px-4 py-3 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-black ${location === '/docs' || location.startsWith('/docs/') ? 'bg-slate-100 dark:bg-black text-primary border-r-4 border-primary' : ''}`}>
          <svg className="w-6 h-6 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
