...
export function NodePanel() {
      ...
      icon: (
        ...
        >
          <path
            ...
            strokeLinejoin="round"
            strokeWidth="2"
            d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z M9 10l0 4 M15 10l0 4"
          />
        </svg>
      ),
    },
  ...
  return (
    <div
      className={`border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-black flex flex-col transition-all duration-300 ${collapsed ? "w-16" : "w-64"}`}
    >
      ...
      <div
        className={`flex-1 overflow-y-auto ${collapsed ? "p-2" : "p-4"} space-y-3`}
      >
        {filteredNodes.map((node) => (
          <div
            ...
            draggable
            onDragStart={(event) =>
              onDragStart(event, node.type, { label: node.name })
            }
          ...
          >
            <div
              className={`bg-white bg-opacity-20 p-2 rounded-lg ${collapsed ? "" : "mr-3"}`}
            >
              {node.icon}
            </div>
            {!collapsed && (
